...
public class DutchNationalFlag {
	...
	public static void dutchFlagPartition(int pivotIndex, List<Color> A) {
		...
		int pivotOrdinal = pivot.ordinal();

		int equal = 0, small = 0, large = A.size() - 1;
		while (equal <= large) {
			int equalOrdinal = A.get(equal).ordinal();
			if (equalOrdinal < pivotOrdinal) {
				Collections.swap(A, small++, equal++);
			} else if (equalOrdinal > pivotOrdinal) {
